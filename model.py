...
class PositionEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        super(PositionEncoding, self).__init__()  # Initialize parent class
        self.d_model = d_model
        self.seq_len = seq_len
        self.dropout = nn.Dropout(dropout)  # Define dropout as nn.Dropout module
        ...
        pe[:, 0::2] = torch.sin(position * den_term)
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        ...
        # how the above operation perfoeming add and norm
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, target_mask):
        ...
        x = self.residual_connection[2](x, lambda x : self.self_attention_block(x,encoder_output,encoder_output,src_mask))
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=256, N: int=6, h: int=2, dropout: float=0.1, d_ff: int=2048) -> Transformer:
