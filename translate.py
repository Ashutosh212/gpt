...
def translate(sentence: str):
    ...
    with torch.no_grad():
        ...
        source = torch.cat([
            torch.tensor([tokenizer_src.token_to_id('[CLS]')], dtype=torch.int64), 
            torch.tensor(source.ids, dtype=torch.int64),
            torch.tensor([tokenizer_src.token_to_id('[SEP]')], dtype=torch.int64),
        ...
        decoder_input = torch.empty(1, 1).fill_(tokenizer_tgt.token_to_id('[CLS]')).type_as(source).to(device)
        ...
        while decoder_input.size(1) < seq_len:
            ...
            if next_word == tokenizer_tgt.token_to_id('[SEP]'):
                break
